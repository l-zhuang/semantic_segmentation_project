...
from models.enet_mish import ENet
...
model = ENet_SAD(num_classes=num_classes,input_size=(512,512), sad=True, encoder_relu=False, decoder_relu=True, weight_share=True,class_weights=class_weights).to(device)
...
train = Train(model, train_loader, optimizer, metric, device)
val = Test(model, val_loader, metric, device)
...
for epoch in range(epochs):
    ...
    if epoch>100:
        epoch_loss, (iou, miou) = train.run_epoch(print_step,True)
        epoch_loss_val, (iou_val, miou_val) = val.run_epoch(print_step,True)
    else:
...
path_name='model_sad.pth'
fig1_name='./pic/TrainingCurve_SAD.png'
fig2_name='./pic/ValidationCurve_SAD.png'
