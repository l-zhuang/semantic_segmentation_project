...
from train_sad import Train
from test_sad import Test
...
from models.enet_mish_sad import ENet
# Default Arguments
height=512
width=512
dataset_dir='./'
...
for epoch in range(epochs):
    ...
    if epoch>70:
        ...
        epoch_loss, (iou, miou) = train.run_epoch(print_step,False)
        epoch_loss_val, (iou_val, miou_val) = val.run_epoch(print_step,False)
    lr_updater.step()
    ...
    if epoch==70:
      model_path = os.path.join('./', 'model_updated_beforesad_70.pth')
      checkpoint = {
          'epoch': epoch,
          'miou': miou,
...
# Show the plot
plt.savefig('./pic/TrainingCurve_withsad_200.png')
plt.show()
...
# Show the plot
plt.savefig('./pic/ValidationCurve_withsad.png')
plt.show()

name = 'model_updated_withsad.pth'
save_dir = './'
